#!/usr/bin/env bash
# run_all_seeds_steplr.sh — Run the full experiment pipeline for multiple seeds
# using the step-LR schedule (MultiStepLR with milestones=[100, 200], gamma=0.1).
#
...
#   nohup bash run_all_seeds_steplr.sh > run_all_seeds_steplr.log 2>&1 &
#   tail -f run_all_seeds_steplr.log        # monitor progress from another terminal
#
# Or to run interactively:
#   bash run_all_seeds_steplr.sh

conda activate /local/data/gme101/thesis_env/

CONFIG="configs/resnet18_cifar10_steplr.yaml"
SEEDS="42 0 1 2 3"
BASE_RESULTS="/local/data/gme101/results_steplr"

echo "============================================================"
echo "Multi-seed pipeline (step LR)"
echo "Seeds:  ${SEEDS}"
echo "Config: ${CONFIG}"
echo "Start:  $(date)"
echo "============================================================"
...
for SEED in ${SEEDS}; do
    ...
    # Output: checkpoints_steplr/seed_{SEED}/ and activations_steplr/seed_{SEED}/
    # ------------------------------------------------------------------
    ...
    # Output: results_steplr/seed_{SEED}/cka_results.csv
    # ------------------------------------------------------------------
    ...
    # Output: results_steplr/seed_{SEED}/drs_results.csv
    ...
    # Output: results_steplr/seed_{SEED}/stabilization_summary.csv
    # ------------------------------------------------------------------
...
" "$SUMMARY_CSV")
    ...
    # Output: activations_steplr/seed_{SEED}/train_full_epoch_{T_STAR}.npy etc.
    # ------------------------------------------------------------------
    ...
    # Output: results_steplr/seed_{SEED}/surrogate_*_epoch_{T_STAR}.csv
    # ------------------------------------------------------------------
